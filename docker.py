...
def mergeSources(name):
    ...
    #Write Video Out
    svout = cv2.VideoWriter(current_event_name + ".avi", fourcc, incoming_FPS, (frame_width, frame_height))
    ...
    time.sleep(0.1)
    #subprocess.call(['ffmpeg', '-loglevel', 'panic', '-i', output_path + name + '.avi', '-i', output_path + name + '.wav', '-c', 'copy', output_path + name + 'm.avi'])
    subprocess.call(['ffmpeg', '-loglevel', 'panic', '-i', name + '.avi', '-i', name + '.wav', '-c', 'copy', output_path + name + 'm.avi'])
...
def recAudio(name):
    ...
    while audioTH:
        ...
        while (len(framebuffer) > int(RATE / CHUNK * int(RECORD_SECONDS + 12))):
            del framebuffer[0]
...
if (rtsp_source == ""):
    try:
        rtsp_source = os.environ['RTSP']
    except:
        print("Failed RTSP:" + rtsp_source)
    print("RTSP:" + rtsp_source)
...
while(True):
    ...
    try:
        ...
        print("moving past empty/invalid frame")
        framedropcnt += 1
        if (framedropcnt > 10):
            audioTH = False
            time.sleep(1)
